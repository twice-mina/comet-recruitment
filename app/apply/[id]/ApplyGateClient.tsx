...
export default function ApplyGateClient() {
  ...
  if (loading) {
    return <div className="max-w-xl mx-auto px-4 py-20 text-center"><p className="text-comet-muted font-body text-sm">Loading...</p></div>;
  }
      ...
      <div className="bg-white border-b border-comet-border">
        <div className="max-w-xl mx-auto px-4 sm:px-6 py-8">
          <nav className="flex items-center gap-1.5 text-xs font-body text-comet-muted mb-5">
            ...
            <span>/</span>
            <Link href={`/jobs/${id}`} className="hover:text-comet-text transition-colors truncate">{job.title}</Link>
            <span>/</span>
          ...
          <p className="text-sm font-body text-comet-muted">
            {job.employer_confidential ? "Confidential Employer" : job.company} · Complete screening to apply via Comet · ~10 minutes
          </p>
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="border border-comet-border rounded-lg p-5 mb-8 bg-comet-surface">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="font-heading font-semibold text-sm text-comet-text">{job.title}</p>
              <p className="text-xs font-body text-comet-muted">
                {job.employer_confidential ? "Confidential Employer" : job.company} · {job.location}
              </p>
            </div>
          </div>
        </div>
        ...
        <p className="text-xs font-body text-comet-muted/60 text-center mt-6">
          Comet Recruitment is not the employer. Screening does not guarantee an interview or offer.
        </p>
      </div>
    </div>
  );
